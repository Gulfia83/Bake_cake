...
def show_main_menu(message) -> None:
    ...
    message.reply_text('''Привет! Я бот bake_cake.
    Благодаря мне ты сможешь заказать торт, о котором ты мечтал.
    Прежде чем произвести заказ, необходимо согласие на обработку персональных данных.''', reply_markup=reply_markup)
...
def get_customization_cakes(query) -> None:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    query.message.reply_text('''Соберите ваш торт.
    Пожалуйста, заполните обязательные поля.
    Мы можем разместить на торте любую надпись, например: "С днем рождения!" за дополнительную плату''', reply_markup=reply_markup)
